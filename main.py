...
class Default(WorkerEntrypoint):
    async def fetch(self, request):
        ...
        # 2. Handle GET (What was causing your current error)
        if request.method != "POST":
            return Response.new(
                "Craftcoin Backend is Online.", 
                headers=CORS_HEADERS,
                status=200
            )

        # 3. Access Storage (self.env instead of just env)
        try:
            storage = self.env.CRAFTCOIN_DATA
        except Exception:
        ...
        try:
            ...
            if action == "create_account":
                ...
                if not username or not password:
                    return Response.new("Error: Missing fields.", headers=CORS_HEADERS, status=400)
